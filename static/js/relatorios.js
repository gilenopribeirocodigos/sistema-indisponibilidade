    ...
    async function gerarRelatorioPorSupervisor() {
        try {
            ...
            data.dados.forEach(sup => {
                ...
                tdSup.innerHTML = `<strong>${sup.supervisor}</strong>`;
                tr.appendChild(tdSup);
                
                // Total eletricistas
                const tdTotal = document.createElement('td');
                tdTotal.textContent = sup.total_eletricistas;
                tr.appendChild(tdTotal);
                
                // Presentes
                const tdPresentes = document.createElement('td');
                tdPresentes.textContent = sup.contadores.Presente || 0;
                ...
                tdTotalReg.innerHTML = `<strong>${sup.total_registros}</strong>`;
                tr.appendChild(tdTotalReg);
                
                // % Presença
                const tdPercPresenca = document.createElement('td');
                const percPresenca = sup.percentuais.Presente || 0;
